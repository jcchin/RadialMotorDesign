...
#TODO: Does not account for the stator winding resistance
...
class Reactance(ExplicitComponent):
    def setup(self):
        self.add_input('m_1', 3, units=None, desc='Number of phases')
        ...
        self.add_input('f', 90, units='Hz', desc='frequency')
        self.add_input('i', 34, units='A', desc='current')  # 'I_a' in Gieras's book
        self.add_input('N_1', 96, units=None, desc='Number of the stator turns per phase')  
        self.add_input('k_w1', 0.95, units=None, desc='Stator winding factor')
        self.add_input('n_m', 20, units=None, desc='Number of poles')  # '2p' in Gieras's book
        self.add_input('tau', 1, units='m', desc='Pole pitch')  # Gieras - pg.134 - (4.27)
        self.add_input('L_i', 0.0345, units='m', desc='Armature stack effective length')
        ...
        self.add_input('pp', 10, units=None, desc='Number of pole pairs')  # 'p' in Gieras's book
...
class Frequency(ExplicitComponent):
    def setup(self):
        self.add_input('rm', 1, units='rpm', desc='motor speed')  # "n_slots" in Gieras's book
...
if __name__ == "__main__":
    ...
    ind.add_output('N_1', 96)  # Stator turns per phase
    ind.add_output('n_m', 20)
    ind.add_output('L_i', 0.0345, units='m')
    ...
    # Frequency:
    ind.add_output('rm', 5400, units='rpm')  # TRY: Sweep across a RPM range?
    ...
    model.add_subsystem('WindingFactor', k_w1(), promotes_inputs=['w_sl', 'm_1', 'n_m', 'n_slots'], promotes_outputs=['pps', 'q_1', 'Q_1', 'k_d1', 'k_p1', 'k_w1'])
    ...
    model.add_subsystem('CartersCoefficient', k_c(), promotes_inputs=['g', 'D_1in', 'n_slots', 'b_14'], promotes_outputs=['mech_angle', 't_1', 'k_c'])
